...
angular.module('ethExplorer')
    .controller('mainCtrl', function($rootScope, $scope, $location) {
        ...
        function updateStats() {
            ...
            try { $scope.versionWhisper = web3.version.whisper; } catch (err) { $scope.versionWhisper = err.message; }
        }
        ...
        function getETHRates() {
            $.getJSON("https://api.coinmarketcap.com/v1/ticker/ethereum/", function(json) {
                var price = Number(json.price_usd);
                $scope.ethprice = "$" + price.toFixed(2);
            });

            $.getJSON("https://api.coinmarketcap.com/v1/ticker/ethereum/", function(json) {
                var btcprice = Number(json.price_btc);
                $scope.ethbtcprice = btcprice;
            });

            $.getJSON("https://api.coinmarketcap.com/v1/ticker/ethereum/", function(json) {
                var cap = Number(json.market_cap_usd);
